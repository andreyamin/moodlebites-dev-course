...
if ($id) {
    $cm         = get_coursemodule_from_id(MOD_PAIRWORK_MODNAME, $id, 0, false, MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    $moduleinstance  = $DB->get_record(MOD_PAIRWORK_TABLE, array('id' => $cm->instance), '*', MUST_EXIST);
} elseif ($n) {
    $moduleinstance  = $DB->get_record(MOD_PAIRWORK_TABLE, array('id' => $n), '*', MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $moduleinstance->course), '*', MUST_EXIST);
    $cm         = get_coursemodule_from_instance(MOD_PAIRWORK_TABLE, $moduleinstance->id, $course->id, false, MUST_EXIST);
} else {
...
if($CFG->version<2014051200){
	...
	$event->add_record_snapshot(MOD_PAIRWORK_MODNAME, $moduleinstance);
	$event->trigger();
} 
...
$renderer = $PAGE->get_renderer(MOD_PAIRWORK_FRANKY);
$reportrenderer = $PAGE->get_renderer(MOD_PAIRWORK_FRANKY,'report');
...
switch ($showreport){
	...
	case 'menu':
		echo $renderer->header($moduleinstance, $cm, $mode, null, get_string('reports', MOD_PAIRWORK_LANG));
	...
	case 'basic':
		$report = new mod_pairwork_basic_report();
